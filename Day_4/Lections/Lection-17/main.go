...
/*
	...
	Program execution
	A complete program is created by linking a single, unimported package called the main package with all 
	the packages it imports, transitively. The main package must have package name main and declare 
...
func main()  {
	...
	var variable int = 30
	var pointer *int = &variable // &... - это операция взятия адреса в памяти
	// Выше создан указатель на переменную variable
	// в pointer лежит 0xc0000120f8 - это место в памяти, где хранится значение(int) 30
	fmt.Printf("Type of pointer: %T\n", pointer)
	fmt.Printf("Value of pointer: %v\n", pointer)
	fmt.Printf("Value of pointer as is: %#v\n", pointer)

	// 3. А какое нулевое значения для указателя
	var zeroPointer *int // zeroPointer имеет значение nil(указатель в никуда)
	fmt.Printf("Type: %T and value: %v and as is: %#v\n", zeroPointer, zeroPointer, zeroPointer)
	if zeroPointer == nil {
		zeroPointer = &variable
	}
	fmt.Printf("After Type: %T and value: %v and as is: %#v\n", zeroPointer, zeroPointer, zeroPointer)

	// 4. Разыменовывание указателя(получение значения)
	// *pointer - возвращает значения, хранимое по адресу
	...
	zeroPtnInt := new(int)
	fmt.Printf("Value of zeroPtnInt: %v and it's address: %v\n", *zeroPtnInt, zeroPtnInt)
	*zeroPtnInt += 40
	fmt.Printf("New value of zeroPtnInt: %v and it's address: %v\n", *zeroPtnInt, zeroPtnInt)
	...
	*c += 99
	...
	// Большой прирост производительности за счето того, что передается не значение,
	// которое должно копироваться, а лишь адрес в памяти, за которым уже хранится какое-то значение
	sample := 1
	...
	newArr := [3]int{1, 2, 3}
	fmt.Println("Before calling mutationSlice:", newArr)
	mutationSlice(newArr[:])
	fmt.Println("After calling mutationSlice:", newArr)
