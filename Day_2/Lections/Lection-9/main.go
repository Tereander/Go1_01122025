...
func main() {
	...
	// Создание массива из 5-ти целочисленных значаний
	// При инициализации массива важно передать информацию - сколько элементов в нём будет
	var arr [5]int
	...
	arr[3] = -100
	arr[4] = 1000
	fmt.Println("After changing:", arr)
	...
	arrWithValues[0] = 1_000
	fmt.Println("Array declaration with [...]:", arrWithValues, len(arrWithValues))

	// 5. Массив - это набор ЗНАЧЕНИЙ. То есть при всех манипуляциях - массив копируются
	...
	// 9. Игнорирование idx при обращении к массиву через range
	var summa float64
	for _, value := range floatArray {
		summa += value
	}
	fmt.Println("Repeat total sum:", math.Round(summa * 100) / 100)
	fmt.Println("Repeat total sum:", math.Round(12.5))  // 13
	fmt.Println("Repeat total sum:", math.Round(11.5))  // 12
	fmt.Println("Repeat total sum:", math.Round(11.49)) // 11
